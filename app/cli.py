...
def _ask_non_empty(prompt: str) -> str:
    while True:
        ...
        if value:
            return value
        print("Warning: you must enter a value.")


def _resolve_extractor(source: str) -> ExtractorFn:
    if source == "sinader":
        return process_sinader
    if source == "sindrep":
        return process_sindrep
    return process_simapro
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Procesa una carpeta con PDFs o XLSX y genera un Excel de salida.",
    )
    parser.add_argument(
        "--source",
        choices=["sinader", "sindrep", "simapro", "ambos"],
        default="ambos",
        help="Tipo de archivo a procesar. Por defecto: ambos.",
    )
    parser.add_argument(
        "--input-dir",
        help="Carpeta que contiene los archivos a procesar (se busca de forma recursiva).",
    )
    parser.add_argument(
        "--output",
...
def main() -> None:
    ...
    input_dir_raw = args.input_dir or _ask_non_empty("Ruta de carpeta a procesar: ")
    input_dir = Path(input_dir_raw).expanduser().resolve()
    if not input_dir.exists() or not input_dir.is_dir():
        raise FileNotFoundError(f"La carpeta no existe o no es valida: {input_dir}")
    ...
    print(f"OK Proceso finalizado. Excel generado en:\n{output}")
